...
export class NilaiController {
  ...
  @Delete('komponen/:id')
  @Roles(Role.PENGAJAR)
  deleteKomponen(@Param('id') id: string, @Request() req: any) {
  ...
  @Post('entry')
  @Roles(Role.PENGAJAR)
  entryNilai(@Body() entryNilaiDto: EntryNilaiDto, @Request() req: any) {
  ...
  getNilaiByKelas(@Param('kelasId') kelasId: string, @Request() req: any) {
    return this.nilaiService.getNilaiByKelas(kelasId, req.user.sub);
  }

  @Get('saya')
  @Roles(Role.PELAJAR)
  getMyNilai(@Request() req: any) {
    return this.nilaiService.getMyNilai(req.user.sub);
  }

  @Delete(':id')
  @Roles(Role.PENGAJAR)
  deleteNilai(@Param('id') id: string, @Request() req: any) {
